...
def get_embeddings(filename, vocab=None, sentiws=None, reduce_dim=None):
    """
    
    :param filename:
    :param vocab:
    :param sentiws: dictionary with sentiws polarity numbers
    :param reduce_dim: dimension to which the word vectors should be scaled down
    :return:
    ...
    with open(filename, "rb") as f:
        ...
        # dimension reduction
        if reduce_dim:
            embedding_dim = reduce_dim
        else:
            reduce_dim = embedding_dim
        ...
        for vocab_idx in range(vocab_size):
            ...
            if word not in vocab:
                ...
                # if init_unknown_words:
                #     vector = np.random.uniform(-0.25, 0.25, int(vector_width/4))
                # else:
                #     continue
                continue
            else:
    ...
    embeddings_index[0] = np.zeros(reduce_dim if sentiws is None else reduce_dim+1)
    embeddings_index['unknown'] = np.random.uniform(-0.25, 0.25, reduce_dim if sentiws is None else reduce_dim+1)
...
def get_embedding_matrix(embeddings_index, embedding_dim, word_index):
    ...
    for word, idx in word_index.items():
        embedding_matrix[idx] = embeddings_index.get(idx, unif_vec)[:embedding_dim]
